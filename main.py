...
def search_for_appointments(
    preferred_store_id=IKEA_STORE_ID,
    ...
    print(
        "🔎  Looking For Appointments @ {} on {} at {}".format(
            preferred_store_id, preferred_date, preferred_slots
        )
    )

    response = requests.get(
        "https://cms.ikea-lsp.de/lsp/entryticket/getslotdata/{}/{}".format(
            preferred_store_id, preferred_date
        ),
        headers=headers,
    )
    ...
    for index, slot in enumerate(slots):
        ...
        if not full_label:
            ...
            print(
                " ... 🗓️  Available {} @ {} {}".format(
                    preferred_date, slot_timing, status
                )
            )
...
def try_booking_slot(preferred_store_id, preferred_date, slot_timing):
    ...
    response = requests.post(
        "https://cms.ikea-lsp.de/lsp/entryticket/submit", headers=headers, data=data
    )
